...
TOTAL_CHANGE = 50 #Number of days to analyze
DECIMAL_VALUE = []
...
def format_stocks_for_analysis():
    ...
    with open("AllStocks/(" + str(datetime.today().date()) + ")Analysis.csv", 'w', 1,
        ...
        for i in companies:
            ...
            try:
                if rh.stocks.get_instruments_by_symbols("SPY")[0]['tradability'] != 'tradable':
                    continue
            except:
                continue
            ...
            if test_date == str(valid_trading_day):
                for j in all_stocks[-(TOTAL_CHANGE + 21):]:
                    if j is not None:
                        if j != all_stocks[-1]:
                            # Stocks must change in value
                            if j["low_price"] == j["high_price"]:
                                break
                            if price == 0:
                                price = j['close_price']
                            else:
...
def template(money):
    for addition in range(0, TOTAL_CHANGE):
        ...
        for i in range(len(stonks)):
            if operations[0](stonks[i][0][19], DECIMAL_VALUE[0]):
    ...
    try:
        ...
        with open(FILE_OUTPUT, "a", 1, newline="") as f:
            ...
            while True:
                for j in range(20):
                    if randint(0, 1) == 1:
                        DECIMAL_VALUE.append(round(random.uniform(number_range[1], 0), 2))
                        operations.append(operator.lt)
                    else:
                        DECIMAL_VALUE.append(round(random.uniform(0, number_range[0]), 2))
    ...
    try:
        instruction = "Buy " + str(msg[1]) + " stocks with an R-squared value of " + str(round(msg[2], 5)) + "." \
            ...
            msg2[4]) + "." + \
                      ...
                      "\n\nThere were " + str(msg2[8]) + " securities analyzed with at least " + str(OPTION_VOLUME) + " volume in options."
        ...
        if value(float(i[0]), 300):
            ...
            for j in range(len(counter)):
                if len(counter[j]) <= int(i[3]) and len(counter[j]) != 0:
                    options = [float(i[0]), counter[j], float(i[1]), i[4], i[5], i[3], TOTAL_CHANGE, i[6], len(companies)]
                    return options
...
def sell_holdings():
    while len(get_portfolio_symbols()) > 1:
        symbols = get_portfolio_symbols()
        for item in symbols:
            if item != 'AMC':
                rh.order_sell_fractional_by_quantity(item,float(rh.build_holdings()[item]['quantity']))
                print('Sold ' + str(item))
                time.sleep(7)


def high_option_liquidity():
    login_robinhood()
    with open("AllWeeklyOptions.csv") as g:
        companies = g.readlines()

    companies = [x.strip() for x in companies]

    with open("AllStocks.csv", 'w', 1,
              newline='') as f:
        writer = csv.writer(f)
        ...
        for i in companies:
            ...
            my_list.clear()
            try:
                # Gets the nearest options chain and extracts it to test2
                test = rh.options.get_chains(str(i))['expiration_dates'][0]
                test2 = rh.find_options_by_expiration((str(i)), test)

                # Separates the options between calls and puts
                for j in test2:
                    if 'type' in j and 'strike_price' in j and 'volume' in j and j['type'] == 'call' and \
                            float(j['strike_price']) >= float(rh.get_latest_price(str(i))[0]):
                        calls.append(j)
                    elif 'type' in j and 'strike_price' in j and 'volume' in j and j['type'] == 'put' and \
                            float(j['strike_price']) <= float(rh.get_latest_price(str(i))[0]):
                        puts.append(j)

                # Sorts the calls and puts lists
                calls.sort(key=lambda x: float(x['strike_price']))
                puts.sort(key=lambda x: float(x['strike_price']), reverse=True)

                # Finds the first 3 calls and puts
                if len(calls) >= 3 and len(puts) >= 3:
                    del calls[3:]
                    del puts[3:]
                    for j in calls:
                        if 'volume' not in j or float(j['volume']) < OPTION_VOLUME:
                            accepted = False
                    for j in puts:
                        if 'volume' not in j or float(j['volume']) < OPTION_VOLUME:
                            accepted = False
...
def main():
    ...
    scheduler2.every().day.at("15:45").do(escape_loop)
    ...
    os.remove("AllStocks/(" + str(datetime.today().date()) + ")DecimalSequences.csv")
    os.remove("AllStocks/(" + str(datetime.today().date()) + ")Analysis.csv")
    scheduler3.every().day.at("18:00").do(high_option_liquidity)
